# Stable Coins Converter
[![Add to Homescreen](https://img.shields.io/badge/Skynet-Add%20To%20Homescreen-00c65e?style=for-the-badge&labelColor=0d0d0d&logo=data:image/png;base64,)](https://homescreen.hns.siasky.net/#/skylink/CADK_qgsiS6J6UbHYCda1TqnZuyqldsiaocLLSsjRy2FsQ)

A dapp where users can swap ETH to DAI and send them to multiple addreses

- HNS decentralized domain = https://increasehappiness.hns.siasky.net/    (Kovan Testnet)

## Skynet Tasks
- [x] Deploy your app’s front-end on Skynet with Homescreen support
- [x] Adding a working "Save to Homescreen" link in the Github repo
- [x] Using an ENS or HNS decentralized domain name for the web app
- [x] Use the Deploy to Skynet Github action for building and deploying your frontend automatically

## Links to example use of Uniswap v3
- Single Swaps (swapExactOutputSingle) https://github.com/ysongh/Stable-Coins-Converter/blob/dev/contracts/StableCoinsConverter.sol#L41
- IQuoter https://github.com/ysongh/Stable-Coins-Converter/blob/dev/contracts/StableCoinsConverter.sol#L72

## Features
- Users can swap ETH with DAI using Uniswap v3 Single Swaps
- Users can send DAI to multiple addresses

## Technologies
- React
- semantic-ui
- Hardhat
- Uniswap v3 ISwapRouter
- Uniswap v3 IQuoter

## Running the dapp on local host
- Clone or download this repository
- Run `npm i` to install the dependencies
- Run `npx hardhat compile`
- Create a file called '.env' on the root folder and add the following code
```
PRIVATEKEY=< Your ETH Address Private key >
ALCHEMYAPI_KEY=< Your Alchemy API key >
```
- Run `npx hardhat run scripts/sample-script.js --network kovan` to deploy the contract to Kovan Testnet
- Run `npm start` to start the dapp